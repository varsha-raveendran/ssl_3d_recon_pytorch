...
class RgbContProj(nn.Module):
    ...
    def forward(self, pcl, feat, N_pts, grid_h, grid_w, well_radius=1., beta=100, mode='rgb'):
        '''
        ...
        mask: bool, (BS,H,W)
                mask of projection
        '''
        add_depth_range = torch.tensor([0,0,1], dtype=torch.float32) # z dim goes from [-1,1] to [0,2]. needed for getting the correct probabilities.
        ...
        # Insert background label at position 0
            ...
            proj_feat = proj_feat * torch.unsqueeze(mask,dim=-1)
        
        return proj_feat, prob, mask            
        
    def get_depth(self, pcl, grid_h, grid_w, N_pts, well_radius=0.5):
        '''
...
class ContProj(nn.Module):
    def __init__(self, ) -> None:
        super().__init__()
    def forward(self, pcl, grid_h, grid_w, N_pts, sigma_sq=0.5):
        '''
