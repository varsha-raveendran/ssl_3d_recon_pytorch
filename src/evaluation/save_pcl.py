...
def optimise_and_save(item, config):
    ...
    img_mask = 1 - \
        torchvision.transforms.Normalize(
            img_mask[0].mean(), img_mask[0].std())(img_mask[0])
    ...
    for epoch in range(num_epochs):
        ...
        pose_ip = torch.unsqueeze(item['random_pose'], 0).to(
            device)  # Batch Size x 2 x 2
        # USE POSENET TO GET POSE PREDICTIONS (B,2)
        pose_out.append(pose_net(img_rgb))
        pose_all = torch.concat(
            [torch.unsqueeze(pose_out[0], axis=1), pose_ip], axis=1)
        ...
        for idx in range(config['n_proj']):
            pcl_xyz, pcl_rgb = recon_net(torch.permute(
                img_out[idx], [0, 3, 1, 2]).contiguous())
            pcl_out.append(pcl_xyz)
            pcl_rgb_out.append(pcl_rgb)

            pose_out.append(pose_net(torch.permute(
                img_out[idx], [0, 3, 1, 2]).contiguous()))

        # Define Losses
        # 2D Consistency Loss - L2
        img_ae_loss, _, _ = img_loss(img_rgb, torch.permute(
            torch.stack(img_out)[0], [0, 3, 1, 2]).contiguous(), 'l2_sq')
        ...
        consist_3d_loss = 0.
        for idx in range(config['n_proj']):
            consist_3d_loss += chamfer_distance(torch.permute(
                pcl_out[idx], [0, 2, 1]), torch.permute(pcl_out[0], [0, 2, 1]))[0]

        # Symmetry loss - assumes symmetry of point cloud about z-axis
        # # Helps obtaining output aligned along z-axis
        pcl_y_pos = (pcl_out[0][:, :, 1:2] > 0).type(
            torch.FloatTensor).to(device)
        pcl_y_neg = (pcl_out[0][:, :, 1:2] < 0).type(
            torch.FloatTensor).to(device)
        ...
        pcl_neg = pcl_y_neg*torch.concat([pcl_out[0][:, :, :1], torch.abs(pcl_out[0][:, :, 1:2]),
                                          pcl_out[0][:, :, 2:3]], -1)
        symm_loss = chamfer_distance(torch.permute(
            pcl_pos, [0, 2, 1]), torch.permute(pcl_neg, [0, 2, 1]))[0]
        # symm_loss = 0.
        # Total Loss
        ...
        recon_loss = (config['lambda_ae']*img_ae_loss) + (config['lambda_3d']*consist_3d_loss)\
            ...
            (config['lambda_mask_fwd']*mask_fwd) + \
            (config['lambda_mask_bwd']*mask_bwd)
        if config["use_symmetry_loss"]:
            recon_loss += (config['lambda_symm']*symm_loss)

        for idx in range(config['n_proj']):
            reg_loss += chamfer_distance(torch.permute(initial_pcl,
                                         [0, 2, 1]), torch.permute(pcl_out[idx], [0, 2, 1]))[0]
